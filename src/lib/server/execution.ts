import { useMessagesStore, type MessageStore, type Step } from "@/store/messagesStore";
...
export function processXmlResponse(response: string, id: string, role: Role): void {
    if (!response) return;
    ...
    const existingMessage = useMessagesStore.getState().messages.findLast(
        (m) => m.id === id
    );
...
export const executeSteps = async (): Promise<void> => {
    ...
    isExecuting = true;
    try {
        const msg = useMessagesStore.getState().messages.findLast(
            (m: MessageStore) => m.steps.length > 0
        );
        if (!msg) {
            return;
        }
        ...
        for (let i = 0; i < updatedMessage.steps.length; i++) {
            ...
            if (step.isPending && step.isComplete) {
                try {
                    console.log(`Executing step ${i + 1}/${updatedMessage.steps.length}: ${step.stepType}`);
        ...
        const pendingExecution = useMessagesStore.getState().messages.findLast(
            (m: MessageStore) => m.steps.some((s: Step) => s.isPending && s.isComplete)
        );
        isExecuting = false;
        if (pendingExecution) {
            executeSteps();
...
async function executeStep(wc: WebContainer, step: Step): Promise<void> {
    switch (step.stepType) {
        ...
        default:
            console.warn('Unknown step type:', (step as { stepType: string }).stepType);
            break;
    }
}
