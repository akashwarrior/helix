...
export function RunCommand({ message }: { message: DataPart["run-command"] }) {
  let title = "";
  if (message.status === "executing") title = "Executing";
  else if (message.status === "waiting") title = "Waiting";
  else if (message.status === "running") title = "Running in background";
  else if (message.status === "error" || message.exitCode === 1)
    title = "Errored";
  else if (message.status === "done") title = "Finished";

  return (
    <ToolMessage>
      <ToolHeader
        title={title}
        icon={<SquareChevronRightIcon className="w-3.5 h-3.5" />}
      />
