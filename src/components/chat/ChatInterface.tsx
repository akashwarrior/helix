...
const MessageStep = memo(({ stepType, content, path, isPending }: { stepType: StepType, content?: string, path?: string, isPending: boolean }) => {
...
export default function ChatInterface({
  ...
  const { messages: msgs, handleInputChange, handleSubmit, reload, status } = useChat({
    api: `/api/chat/${chatId}`,
  ...
  return (
    <div
      className={cn(
        "overflow-hidden bg-card/70 transition-all duration-300 h-full flex flex-col",
