Use this tool to run a command inside a Sandbox. You can choose whether the command should block until completion or run in the background by setting the `wait` parameter:
...
2. You want to run a build or test process (e.g., `pnpm build`)
3. You need to compile or execute code within the sandbox
4. You want to run a task in the background without blocking the session
...
- Separate the base command from its arguments
  - ✅ `{ command: "pnpm", args: ["install", "package"], wait: true }`
